...
{
    class FFLUSH : ExpressionBase
    {
        ExpressionBase handleExpression;
        ExpressionBase forceExpression;
        ...
        override internal Variant GetVariant(CallingContext context)
        {
            ...
            if (forceExpression != null)
            {
                value = forceExpression.GetVariant(context);
                if (value.IsNull)
                {
                    throw new ErrorException(ErrorCodes.InvalidArgument);
                }
                // Only strings are valid parameters
                if (value.Type != VariantType.Logical)
                {
                    throw new ErrorException(ErrorCodes.InvalidArgument);
                }
            }
        ...
        internal override bool GetBool(CallingContext context)
        {
            ...
            bool bForce = false;
            if (forceExpression != null)
                bForce = forceExpression.GetBool(context);

            LowLevelFileMngr llfmngr = GuineuInstance.FFilesManager;
            bool retVal = llfmngr.FFlush(guineuHandle, bForce);
