...
{
	class DesktopImage : PictureBox, IControl
	{
		readonly ImageHandling img;

		public DesktopImage()
		{
			img = new ImageHandling(this);
		}
		...
		public Variant GetVariant(KnownNti nti)
		{
			switch (nti)
			{
				...
				case KnownNti.Picture:
					return new Variant(img.Picture);

				case KnownNti.PictureVal:
					return new Variant(img.PictureVal);

				case KnownNti.Stretch:
					return new Variant(img.Stretch, 10);
